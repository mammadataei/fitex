...
export const createCampaignNameSchema = (existingCampaigns: Campaign[] = []) =>
  z
    ...
    .refine((name) => {
      const nameExists = existingCampaigns.some((campaign) => campaign.name.toLowerCase() === name.toLowerCase());
      return !nameExists;
...
export const validateCampaignName = (name: string, existingCampaigns: Campaign[] = []): string | null => {
